...
func GetChequeFrequency(distrib_id string) (int, *gorm.DB) {
	var frequency int
...
func IncrementCheckoutFrequency(distrib_id string, frequency int) *gorm.DB {
	result := configs.DB.Model(models.ChequeFrequencies{}).Where("distrib_id=?", distrib_id).Update("frequency", frequency+1)
	return result
}
