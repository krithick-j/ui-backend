...
func UpdateFinalBvToTc(distrib_id string, place string, finalTotalBV int, tx *gorm.DB) *gorm.DB {
	result := tx.Model(models.TrackingCenter{}).Where("distrib_id=? AND place=?", distrib_id, place).Update("bv", finalTotalBV)
	return result
}
...
func UpdateParentPlaceBv(distrib_id string, placeBv dto.PlaceBv, tx *gorm.DB) (*gorm.DB, int) {
	var currentBv int
	...
	result := tx.Table("tracking_centers").Where("distrib_id=? AND place=?", distrib_id, placeBv.Place).Update("bv", updatedBv)
	if result.Error != nil {
...
func UpdateLeftPointPlaceBv(distrib_id string, placeBv dto.PlaceBv,  tx *gorm.DB) (*gorm.DB, int) {
	...
	result := tx.Table("tracking_centers").Where("distrib_id=? AND place=?", distrib_id, placeBv.Place).Update("left_point", currentLeftPoint)
	return result, currentLeftPoint
}

func UpdateRightPointPlaceBv(distrib_id string, placeBv dto.PlaceBv, tx *gorm.DB) (*gorm.DB, int) {
	...
	result := tx.Table("tracking_centers").Where("distrib_id=? AND place=?", distrib_id, placeBv.Place).Update("right_point", currentRightPoint)
	return result, currentRightPoint
}

func UpdateTrackingCenter(leftPoint int, rightPoint int, distrib_id string, place string, tx *gorm.DB) *gorm.DB {
	result := tx.Table("tracking_centers").Where("distrib_id=? AND place=?", distrib_id, place).Updates(map[string]interface{}{"left_point": leftPoint, "right_point": rightPoint})
	return result
}
